...
export default function DesktopView({ data, offers, viewMode, tableTopOffset, footerHeight, setViewMode }: DesktopViewProps) {
    ...
    // Viste che si adattano al cambio di modalità. 
    useEffect(() => {
        // Se si entra in Vista Sintetica, chiude tutte le categorie.
        if (viewMode === 'summary') {
            setOpenCategories(Object.fromEntries(data.categorieCoperture.map(c => [c.nome, false])));
        }
        // Se si entra in Vista Completa, apre tutte le categorie.
        else if (viewMode === 'full') {
            setOpenCategories(Object.fromEntries(data.categorieCoperture.map(c => [c.nome, true])));
        }
        // In Vista Compatta, non fa nulla, lasciando il controllo all'utente.
    ...
    return (
        <div className="hidden md:block" id="print-area">
            ...
            >
                ...
                >
                    {/* Il resto del codice JSX rimane invariato */}
                    <div className="sticky top-0 left-0 z-30 bg-sidebar border-r border-b flex flex-col items-center justify-center p-3 text-center h-[7.5rem]">
